# GondalPortfolio
Liaba Gondals Design Portfolio 2026




***Project 1:***
HTML & CSS webpage

https://gondaliaba.github.io/GondalPortfolio/

Info: Created a webpage for a fictional restaurant using HTML and CSS focusing on layout and user-friendly navigation. 




***Project 2:***
Figma Homepage Redesign (click present) 

https://www.figma.com/design/0G3d5bRh99HdOKGpygZuNM/DyaTech-Website?node-id=0-1&t=LfviSl9T0cz896f8-1

Info: Using Figma, I redesigned DyaTech's homepage to simplify the layout and reduce visual clutter. 




***Project 3:***
Figma Etsy App Redesign 

https://drive.google.com/file/d/1_duDakS6JSMPq2L1kDgC-INMCub0rZPs/view?usp=sharing

Liaba designed the following pages: "Top Seller Spotlight", "Thread & Thistle", "Recent Searches", "Cart"

Info: (Group Project) Redesigned Etsy's interface to reduce clutter and improve product discovery. Introduced "Seller's Week" promotion and "Spotlight" feature. 




***Project 4:***
DyaTech Orion Mock Up 

https://www.figma.com/proto/JsWW3RHnUogrGjCmM5Tpeh/Orion-High-fidelity-mockups?page-id=2834%3A37648&node-id=3897-57603&node-type=canvas&viewport=569%2C781%2C0.04&t=evGcUiTTKlehtIT4-1&scaling=min-zoom&content-scaling=fixed&starting-point-node-id=3897%3A57603&show-proto-sidebar=1

Liaba designed the "Support Center" page. 

Info: Collaborated on a DyaTech Orion Catolog's website mockup for the NYC DOE, creating a clean, user-friendly interface to improve user navigation. 




***Project 5:***
Python Coding Project "beautifulsoupproject.py" 

(view in files)

Info: Python project using BeautifulSoup to scrape book titles from a website in order to practice web scraping and data extraction. 


